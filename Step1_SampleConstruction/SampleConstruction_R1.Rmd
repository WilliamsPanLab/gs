...
```{r}
###################∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆##############
##  Chunk 11 saves out tertile thresholds for boots  ##
###################∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆∆##############
...
effect_range <- with(data, seq(0, p99), length.out = 100)
...
effect_range <- with(data, seq(0, i99), length.out = 100)
...
effect_range <- with(data, seq(0, e99), length.out = 100)
...
```{r}
# BPM robustness analysis - internal

```

```{r}
# BPM robustness analysis - external

```

```{r}

# evaluate impact of including bpm vs. cbcl
fullMod=gam(g~s(cbcl_scr_syn_totprob_r,k=4)+s(interview_age,k=4),data=masterdf_BPM)
reducedMod=gam(g~s(bpTot,k=4)+s(interview_age,k=4),data=masterdf_BPM)
# int
fullMod=gam(g~s(cbcl_scr_syn_internal_r,k=4)+s(interview_age,k=4),data=masterdf_BPM)
reducedMod=gam(g~poly(bpInt,2)+s(interview_age,k=4),data=masterdf_BPM)
# ext
fullMod=gam(g~s(cbcl_scr_syn_external_r,k=4)+s(interview_age,k=4),data=masterdf_BPM)
reducedMod=gam(g~s(bpExt,k=4)+s(interview_age,k=4),data=masterdf_BPM)
```
