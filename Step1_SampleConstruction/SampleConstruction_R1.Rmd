...
# extract for plot
plot_data <- masterdf_pub_filtered[, c("pds_p_ss_male_category", "pds_p_ss_female_category","g")]

# Plot using ggplot
library(ggplot2)
ggplot(data=plot_data, aes(x = pds_p_ss_female_category, y = g)) +
  geom_boxplot()

# model
model<-gam(g~s(cbcl_scr_syn_internal_r)+stage,data=masterdf_pub_filtered)

# girl model
model<-gam(g~s(cbcl_scr_syn_internal_r)+pds_p_ss_female_category,data=masterdf_pub_filtered)

# boy model
model<-gam(g~s(cbcl_scr_syn_internal_r)+pds_p_ss_male_category,data=masterdf_pub_filtered)
