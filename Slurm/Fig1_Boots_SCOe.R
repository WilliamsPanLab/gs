...
# load in data
masterdf=readRDS('/oak/stanford/groups/leanew1/users/apines/data/gp/gp_SCOe_masterdf.rds')
# FYI
...
for (b in 1:1000){
	...
	bpmax=max(bootSamp$cbcl_scr_syn_external_r)
	######## II PREDICT VARIABLE OF INTEREST WITH FIT SPLINE
	#### g as response variable
	pgAge<-lm(g~cbcl_scr_syn_external_r+interview_age,data=bootSamp)
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/gp_SCOe_Boots.rds')
