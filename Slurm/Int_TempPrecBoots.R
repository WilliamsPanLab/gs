...
masterdf=masterdf[,c('cbcl_scr_syn_internal_r.x','cbcl_scr_syn_internal_r.y','ple_died_y.x','ple_injured_y.x','ple_crime_y.x','ple_friend_y.x','ple_friend_injur_y.x','ple_arrest_y.x','ple_friend_died_y.x','ple_mh_y.x','ple_sib_y.x','ple_victim_y.x','ple_separ_y.x','ple_law_y.x','ple_school_y.x','ple_move_y.x','ple_jail_y.x','ple_step_y.x','ple_new_job_y.x','ple_new_sib_y.x','g.x','subjectkey','interview_age.x','Grades.x','parentPcount.x','income.x','parental_education.x','sex.x','race_ethnicity.x','weight.x','waist.x','height.x','BMI.x')]
...
for (b in 1:10000){
	...
	sumSq_full[b]=sum(((exp(predictFull)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predictFullHeldOut=predict.bam(fullModel,heldOut)
	sumSq_heldout_full[b]=sum(((exp(predictFullHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_full[b]=median(abs((exp(predictFull)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	...
	sumSq_n_died[b]=sum(((exp(predict_n_died)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_diedHeldOut=predict.bam(Model_n_died,heldOut)
	sumSq_heldout_n_died[b]=sum(((exp(predict_n_diedHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	sumSq_n_injured[b]=sum(((exp(predict_n_injured)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_injuredHeldOut=predict.bam(Model_n_injured,heldOut)
	sumSq_heldout_n_injured[b]=sum(((exp(predict_n_injuredHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_injured[b]=median(abs((exp(predict_n_injured)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_injured[b]=median(abs((exp(predict_n_injuredHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_crime[b]=sum(((exp(predict_n_crime)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_crimeHeldOut=predict.bam(Model_n_crime,heldOut)
	sumSq_heldout_n_crime[b]=sum(((exp(predict_n_crimeHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_crime[b]=median(abs((exp(predict_n_crime)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_crime[b]=median(abs((exp(predict_n_crimeHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_friend[b]=sum(((exp(predict_n_friend)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_friendHeldOut=predict.bam(Model_n_friend,heldOut)
	sumSq_heldout_n_friend[b]=sum(((exp(predict_n_friendHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_friend[b]=median(abs((exp(predict_n_friend)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_friend[b]=median(abs((exp(predict_n_friendHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_friend_injur[b]=sum(((exp(predict_n_friend_injured)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_friend_injuredHeldOut=predict.bam(Model_n_friend_injured,heldOut)
	sumSq_heldout_n_friend_injur[b]=sum(((exp(predict_n_friend_injuredHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_friend_injur[b]=median(abs((exp(predict_n_friend_injured)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_friend_injur[b]=median(abs((exp(predict_n_friend_injuredHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_arrest[b]=sum(((exp(predict_n_arrest)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_arrestHeldOut=predict.bam(Model_n_arrest,heldOut)
	sumSq_heldout_n_arrest[b]=sum(((exp(predict_n_arrestHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_arrest[b]=median(abs((exp(predict_n_arrest)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_arrest[b]=median(abs((exp(predict_n_arrestHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_friend_died[b]=sum(((exp(predict_n_friend_died)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_friend_diedHeldOut=predict.bam(Model_n_friend_died,heldOut)
	sumSq_heldout_n_friend_died[b]=sum(((exp(predict_n_friend_diedHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_friend_died[b]=median(abs((exp(predict_n_friend_died)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_friend_died[b]=median(abs((exp(predict_n_friend_diedHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_mh[b]=sum(((exp(predict_n_mh)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_mhHeldOut=predict.bam(Model_n_mh,heldOut)
	sumSq_heldout_n_mh[b]=sum(((exp(predict_n_mhHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_mh[b]=median(abs((exp(predict_n_mh)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_mh[b]=median(abs((exp(predict_n_mhHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_sib[b]=sum(((exp(predict_n_sib)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_sibHeldOut=predict.bam(Model_n_sib,heldOut)
	sumSq_heldout_n_sib[b]=sum(((exp(predict_n_sibHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_sib[b]=median(abs((exp(predict_n_sib)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_sib[b]=median(abs((exp(predict_n_sibHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_victim[b]=sum(((exp(predict_n_victim)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_victimHeldOut=predict.bam(Model_n_victim,heldOut)
	sumSq_heldout_n_victim[b]=sum(((exp(predict_n_victimHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_victim[b]=median(abs((exp(predict_n_victim)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_victim[b]=median(abs((exp(predict_n_victimHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_separ[b]=sum(((exp(predict_n_separ)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_separHeldOut=predict.bam(Model_n_separ,heldOut)
	sumSq_heldout_n_separ[b]=sum(((exp(predict_n_separHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_separ[b]=median(abs((exp(predict_n_separ)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_separ[b]=median(abs((exp(predict_n_separHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_law[b]=sum(((exp(predict_n_law)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_lawHeldOut=predict.bam(Model_n_law,heldOut)
	sumSq_heldout_n_law[b]=sum(((exp(predict_n_lawHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_law[b]=median(abs((exp(predict_n_law)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_law[b]=median(abs((exp(predict_n_lawHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_school[b]=sum(((exp(predict_n_school)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_schoolHeldOut=predict.bam(Model_n_school,heldOut)
	sumSq_heldout_n_school[b]=sum(((exp(predict_n_schoolHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_school[b]=median(abs((exp(predict_n_school)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_school[b]=median(abs((exp(predict_n_schoolHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_move[b]=sum(((exp(predict_n_move)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_moveHeldOut=predict.bam(Model_n_move,heldOut)
	sumSq_heldout_n_move[b]=sum(((exp(predict_n_moveHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_move[b]=median(abs((exp(predict_n_move)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_move[b]=median(abs((exp(predict_n_moveHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_jail[b]=sum(((exp(predict_n_jail)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_jailHeldOut=predict.bam(Model_n_jail,heldOut)
	sumSq_heldout_n_jail[b]=sum(((exp(predict_n_jailHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_jail[b]=median(abs((exp(predict_n_jail)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_jail[b]=median(abs((exp(predict_n_jailHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))

	### step
	Model_n_step<-bam(cbcl_scr_syn_internal_r.y~cbcl_scr_syn_internal_r.x+ple_died_y.x+ple_injured_y.x+ple_crime_y.x+ple_friend_y.x+ple_friend_injur_y.x+ple_arrest_y.x+ple_friend_died_y.x+ple_mh_y.x+ple_sib_y.x+ple_victim_y.x+ple_separ_y.x+ple_law_y.x+ple_school_y.x+ple_move_y.x+ple_jail_y.x+ple_new_job_y.x+ple_new_sib_y.x+s(g.x,k=4)+s(interview_age.x,k=4)+s(Grades.x,k=4)+s(parentPcount.x,k=4)+income.x+s(parental_education.x,k=4)+sex.x+race_ethnicity.x+s(weight.x,k=4)+s(waist.x,k=4)+s(height.x,k=4)+s(BMI.x,k=4),data=bootSamp)
	# predict to get sum of squares
	predict_n_step=predict.bam(Model_n_step,bootSamp)
	sumSq_n_step[b]=sum(((exp(predict_n_step)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_stepHeldOut=predict.bam(Model_n_step,heldOut)
	sumSq_heldout_n_step[b]=sum(((exp(predict_n_stepHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_step[b]=median(abs((exp(predict_n_step)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_step[b]=median(abs((exp(predict_n_stepHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_new_job[b]=sum(((exp(predict_n_job)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_jobHeldOut=predict.bam(Model_n_job,heldOut)
	sumSq_heldout_n_new_job[b]=sum(((exp(predict_n_jobHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_new_job[b]=median(abs((exp(predict_n_job)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_new_job[b]=median(abs((exp(predict_n_jobHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_new_sib[b]=sum(((exp(predict_n_new_sib)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_new_sibHeldOut=predict.bam(Model_n_new_sib,heldOut)
	sumSq_heldout_n_new_sib[b]=sum(((exp(predict_n_new_sibHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_new_sib[b]=median(abs((exp(predict_n_new_sib)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get MAE held out
	hMAE_n_new_sib[b]=median(abs((exp(predict_n_new_sibHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_g[b]=sum(((exp(predict_n_g)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_gHeldOut=predict.bam(Model_n_g,heldOut)
	sumSq_heldout_n_g[b]=sum(((exp(predict_n_gHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# MAE
	MAE_n_g[b]=median(abs((exp(predict_n_g)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_g[b]=median(abs((exp(predict_n_gHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_interview_age[b]=sum(((exp(predict_n_age)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_ageHeldOut=predict.bam(Model_n_age,heldOut)
	sumSq_heldout_n_interview_age[b]=sum(((exp(predict_n_ageHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# MAE
	MAE_n_interview_age[b]=median(abs((exp(predict_n_age)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_interview_age[b]=median(abs((exp(predict_n_ageHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_Grades[b]=sum(((exp(predict_n_grades)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_gradesHeldOut=predict.bam(Model_n_grades,heldOut)
	sumSq_heldout_n_Grades[b]=sum(((exp(predict_n_gradesHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# MAE
	MAE_n_Grades[b]=median(abs((exp(predict_n_grades)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_Grades[b]=median(abs((exp(predict_n_gradesHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_parentPcount[b]=sum(((exp(predict_n_parentPcount)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_parentPcountHeldOut=predict.bam(Model_n_parentPcount,heldOut)
	sumSq_heldout_n_parentPcount[b]=sum(((exp(predict_n_parentPcountHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# MAE
	MAE_n_parentPcount[b]=median(abs((exp(predict_n_parentPcount)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_parentPcount[b]=median(abs((exp(predict_n_parentPcountHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_income[b]=sum(((exp(predict_n_income)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_incomeHeldOut=predict.bam(Model_n_income,heldOut)
	sumSq_heldout_n_income[b]=sum(((exp(predict_n_incomeHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# MAE
	MAE_n_income[b]=median(abs((exp(predict_n_income)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_income[b]=median(abs((exp(predict_n_incomeHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_parental_education[b]=sum(((exp(predict_n_parental_education)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_parental_educationHeldOut=predict.bam(Model_n_parental_education,heldOut)
	sumSq_heldout_n_parental_education[b]=sum(((exp(predict_n_parental_educationHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# MAE
	MAE_n_parental_education[b]=median(abs((exp(predict_n_parental_education)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_parental_education[b]=median(abs((exp(predict_n_parental_educationHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_sex[b]=sum(((exp(predict_n_sex)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_sexHeldOut=predict.bam(Model_n_sex,heldOut)
	sumSq_heldout_n_sex[b]=sum(((exp(predict_n_sexHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_sex[b]=median(abs((exp(predict_n_sex)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# MAE held out
	hMAE_n_sex[b]=median(abs((exp(predict_n_sexHeldOut)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	sumSq_n_race_ethnicity[b]<-sum(((exp(predict_n_race_ethnicity)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
	# predict held out to get sum of squares on held out
	predict_n_race_ethnicityHeldout=predict.bam(Model_n_race_ethnicity,heldOut)
	sumSq_heldout_n_race_ethnicity[b]<-sum(((exp(predict_n_race_ethnicityHeldout-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
	# get AIC
	...
	# get MAE
	MAE_n_race_ethnicity[b]=median(abs((exp(predict_n_race_ethnicity)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get hMAE
	hMAE_n_race_ethnicity[b]=median(abs((exp(predict_n_race_ethnicity)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	# predict to get sum of squares
        ...
        sumSq_n_weight[b]=sum(((exp(predict_n_weight)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
        # predict held out to get sum of squares on held out
        predict_n_weightHeldOut=predict.bam(Model_n_weight,heldOut)
        sumSq_heldout_n_weight[b]=sum(((exp(predict_n_weightHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
        # get AIC
	...
	# get MAE
	MAE_n_weight[b]=median(abs((exp(predict_n_weight)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get hMAE
	hMAE_n_weight[b]=median(abs((exp(predict_n_weight)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	# predict to get sum of squares
        ...
        sumSq_n_waist[b]=sum(((exp(predict_n_waist)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
        # predict held out to get sum of squares on held out
        predict_n_waistHeldOut=predict.bam(Model_n_waist,heldOut)
        sumSq_heldout_n_waist[b]=sum(((exp(predict_n_waistHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
        # get AIC
	...
	# get MAE
	MAE_n_waist[b]=median(abs((exp(predict_n_waist)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get hMAE
	hMAE_n_waist[b]=median(abs((exp(predict_n_waist)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	# predict to get sum of squares
        ...
        sumSq_n_height[b]=sum(((exp(predict_n_height)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
        # predict held out to get sum of squares on held out
        predict_n_heightHeldOut=predict.bam(Model_n_height,heldOut)
        sumSq_heldout_n_height[b]=sum(((exp(predict_n_heightHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
        # get AIC
	...
	# get MAE
	MAE_n_height[b]=median(abs((exp(predict_n_height)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get hMAE
	hMAE_n_height[b]=median(abs((exp(predict_n_height)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
	...
	# predict to get sum of squares
        ...
        sumSq_n_BMI[b]=sum(((exp(predict_n_BMI)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1))^2)
        # predict held out to get sum of squares on held out
        predict_n_BMIHeldOut=predict.bam(Model_n_BMI,heldOut)
        sumSq_heldout_n_BMI[b]=sum(((exp(predict_n_BMIHeldOut-heldOut$cbcl_scr_syn_internal_r.y)-1))^2)
        # get AIC
	...
	# get MAE
	MAE_n_BMI[b]=median(abs((exp(predict_n_BMI)-1)-(exp(bootSamp$cbcl_scr_syn_internal_r.y)-1)))
	# get hMAE
	hMAE_n_BMI[b]=median(abs((exp(predict_n_BMI)-1)-(exp(heldOut$cbcl_scr_syn_internal_r.y)-1)))
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_sumSq.rds')
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_sumSqHeldout.rds')
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_DevExplained.rds')
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_AIC.rds')
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_BIC.rds')
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_number.rds')
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_MAE.rds')
# saveout heldout MAE
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_hMAE.rds')

# set colnames of betas prior to saveout (from last iteration)
colnames(stdbetas)=namesOfint
saveRDS(stdbetas,'/oak/stanford/groups/leanew1/users/apines/data/gp/Int_TempPrec_betas.rds')
