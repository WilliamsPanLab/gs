...
for (b in 1:3000){
	...
	### step
	Model_n_step<-bam(cbcl_scr_syn_totprob_r~ple_died_y+ple_injured_y+ple_crime_y+ple_friend_y+ple_friend_injur_y+ple_arrest_y+ple_friend_died_y+ple_mh_y+ple_sib_y+ple_victim_y+ple_separ_y+ple_law_y+ple_school_y+ple_move_y+ple_jail_y+ple_new_job_y+ple_new_sib_y+s(g,k=4)+s(interview_age,k=4)+s(Grades,k=4)+s(parentPcount,k=4)+s(income,k=4)+s(parental_education,k=4)+sex+race_ethnicity+s(weight,k=4)+s(waist,k=4)+s(height,k=4)+s(BMI,k=4),data=bootSamp)
	# predict to get sum of squares
	...
	### new job
	Model_n_job<-bam(cbcl_scr_syn_totprob_r~ple_died_y+ple_injured_y+ple_crime_y+ple_friend_y+ple_friend_injur_y+ple_arrest_y+ple_friend_died_y+ple_mh_y+ple_sib_y+ple_victim_y+ple_separ_y+ple_law_y+ple_school_y+ple_move_y+ple_jail_y+ple_step_y+ple_new_sib_y+s(g,k=4)+s(interview_age,k=4)+s(Grades,k=4)+s(parentPcount,k=4)+s(income,k=4)+s(parental_education,k=4)+sex+race_ethnicity+s(weight,k=4)+s(waist,k=4)+s(height,k=4)+s(BMI,k=4),data=bootSamp)
	# predict to get sum of squares
	...
	# get MAE in heldout
	hMAE_n_sex[b]=median(abs((exp(predict_n_sex)-1)-(exp(heldOut$cbcl_scr_syn_totprob_r)-1)))
	...
	# get MAE in heldout
	hMAE_n_race_ethnicity[b]=median(abs((exp(predict_n_race_ethnicity)-1)-(exp(heldOut$cbcl_scr_syn_totprob_r)-1)))
	...
	# get MAE in heldout
	hMAE_n_weight[b]=median(abs((exp(predict_n_weight)-1)-(exp(heldOut$cbcl_scr_syn_totprob_r)-1)))
	...
	# get MAE in heldout
	hMAE_n_waist[b]=median(abs((exp(predict_n_waist)-1)-(exp(heldOut$cbcl_scr_syn_totprob_r)-1)))
	...
	# get MAE
	MAE_n_height[b]=mean(abs(exp(predict_n_heightHeldOut)-1-exp(heldOut$cbcl_scr_syn_totprob_r)+1))
	# get MAE in heldout
	hMAE_n_height[b]=median(abs((exp(predict_n_height)-1)-(exp(heldOut$cbcl_scr_syn_totprob_r)-1)))
	...
	# get MAE
	MAE_n_BMI[b]=mean(abs(exp(predict_n_BMIHeldOut)-1-exp(heldOut$cbcl_scr_syn_totprob_r)+1))
	# get MAE in heldout
	hMAE_n_BMI[b]=median(abs((exp(predict_n_BMI)-1)-(exp(heldOut$cbcl_scr_syn_totprob_r)-1)))
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/p_MAE.rds')
# saveout heldout MAE
