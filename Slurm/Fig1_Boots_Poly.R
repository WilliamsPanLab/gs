...
for (b in 1:1000){
	...
	pgAge<-bam(g~poly(cbcl_scr_syn_totprob_r,2)+interview_age,data=bootSamp)
	intgAge<-lm(g~poly(cbcl_scr_syn_internal_r,2)+interview_age,data=bootSamp)
	extgAge<-lm(g~poly(cbcl_scr_syn_external_r,2)+interview_age,data=bootSamp)
	somgAge<-lm(g~poly(cbcl_scr_syn_somatic_r,2)+interview_age,data=bootSamp)
	anxgAge<-lm(g~poly(cbcl_scr_syn_anxdep_r,2)+interview_age,data=bootSamp)
	thogAge<-lm(g~poly(cbcl_scr_syn_thought_r,2)+interview_age,data=bootSamp)
	witgAge<-lm(g~poly(cbcl_scr_syn_withdep_r,2)+interview_age,data=bootSamp)
	socgAge<-lm(g~poly(cbcl_scr_syn_social_r,2)+interview_age,data=bootSamp)
	attgAge<-lm(g~poly(cbcl_scr_syn_attention_r,2)+interview_age,data=bootSamp)
	rulgAge<-lm(g~poly(cbcl_scr_syn_rulebreak_r,2)+interview_age,data=bootSamp)
	agggAge<-lm(g~poly(cbcl_scr_syn_aggressive_r,2)+interview_age,data=bootSamp)
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/gpBoots_poly_1k.rds')
outdf=data.frame(pDeriv,intDeriv,extDeriv,somDeriv,anxDeriv,thoDeriv,witDeriv,socDeriv,attDeriv,rulDeriv,aggDeriv)
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/gpDerivBoots_poly_1k.rds')
outdf=data.frame(pFit,intFit,extFit,somFit,anxFit,thoFit,witFit,socFit,attFit,rulFit,aggFit)
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/gpFitBoots_poly_1k.rds')
