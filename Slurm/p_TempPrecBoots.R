...
for (b in 1:5000){
	...
	### step
	Model_n_step<-bam(cbcl_scr_syn_totprob_r.y~cbcl_scr_syn_totprob_r.x+ple_died_y.x+ple_injured_y.x+ple_crime_y.x+ple_friend_y.x+ple_friend_injur_y.x+ple_arrest_y.x+ple_friend_died_y.x+ple_mh_y.x+ple_sib_y.x+ple_victim_y.x+ple_separ_y.x+ple_law_y.x+ple_school_y.x+ple_move_y.x+ple_jail_y.x+ple_new_job_y.x+ple_new_sib_y.x+s(g.x,k=4)+s(interview_age.x,k=4)+s(Grades.x,k=4)+s(parentPcount.x,k=4)+s(income.x,k=4)+s(parental_education.x,k=4)+sex.x+race_ethnicity.x+s(weight.x,k=4)+s(waist.x,k=4)+s(height.x,k=4)+s(BMI.x,k=4),data=bootSamp,family=nb())
	# predict to get sum of squares
