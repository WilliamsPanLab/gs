...
for (b in 1:10000){
	...
	bdepmax=max(bootSamp$cbcl_scr_syn_depress_r)
	...
	eachIntcount=seq(0:binmax)
	eachExtcount=seq(0:bexmax)
	eachSomcount=seq(0:bsommax)
	eachAnxcount=seq(0:banxmax)
	eachThocount=seq(0:bthommax)
	eachWitDepcount=seq(0:bwitmax)
	...
	predictDFi=data.frame(eachIntcount,rep(median(bootSamp$interview_age),(binmax+1)))
	predictDFe=data.frame(eachExtcount,rep(median(bootSamp$interview_age),(bexmax+1)))
	...
	predictDFt=data.frame(eachThocount,rep(median(bootSamp$interview_age),(bthommax+1)))
	predictDFwd=data.frame(eachWitDepcount,rep(median(bootSamp$interview_age),(bwitmax+1)))
	...
	predictDF_segTho=data.frame(eachThocount,rep(median(bootSamp$interview_age),(bthommax+1)),rep("F",(bthommax+1)))
	predictDF_povTho=data.frame(eachThocount,rep(median(bootSamp$interview_age),(bthommax+1)),rep("1",(bthommax+1)))
	predictDF_segWitDep=data.frame(eachWitDepcount,rep(median(bootSamp$interview_age),(bwitmax+1)),rep("F",(bwitmax+1)))
	predictDF_povWitDep=data.frame(eachWitDepcount,rep(median(bootSamp$interview_age),(bwitmax+1)),rep("1",(bwitmax+1)))
	...
	fotfitint_P=predict(IntgAge_pov,predictDF_povInt)
