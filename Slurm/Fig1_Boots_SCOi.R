...
# load in data
masterdf=readRDS('/oak/stanford/groups/leanew1/users/apines/data/gp/gp_SCOi_masterdf.rds')
# FYI
...
for (b in 1:50){
	...
	bpmax=max(bootSamp$cbcl_scr_syn_internal_r)
	######## II PREDICT VARIABLE OF INTEREST WITH FIT SPLINE
	#### g as response variable
	pgAge<-lm(g~cbcl_scr_syn_internal_r+interview_age,data=bootSamp)
...
saveRDS(outdf,'/oak/stanford/groups/leanew1/users/apines/data/gp/gp_SCOi_Boots.rds')
