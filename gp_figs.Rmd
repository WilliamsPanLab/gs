...
d=masterdf[!masterdf$cbcl_q61_p=='',]
...
```{r}
### PRINOUT BIG DATA FOR CLUSTER MIXED EFFECTS
# save out model for cluster mixed-effect fitting
...
ggplot(aes(x=variable,y=value),data=boot_barplot_df)+geom_boxplot()+theme(axis.text.x=element_text(angle=45,vjust=.5))+theme_classic()+ggtitle('Total Problems')

boots<-readRDS('~/Desktop/g_p/DevExplainedInt.rds')
boot_barplot_df<-melt(boots)
ggplot(aes(x=variable,y=value),data=boot_barplot_df)+geom_boxplot()+theme(axis.text.x=element_text(angle=45,vjust=.5))+theme_classic()+ggtitle('Internalizing Problems')

boots<-readRDS('~/Desktop/g_p/DevExplainedExt.rds')
boot_barplot_df<-melt(boots)
ggplot(aes(x=variable,y=value),data=boot_barplot_df)+geom_boxplot()+theme(axis.text.x=element_text(angle=45,vjust=.5))+theme_classic()+ggtitle('Externalizing Problems')
```
...
ASRcolnames=asr[1,]
####### QUICK ASIDE TO TRY MICHELI APPROACH TO FACTOR DELINEATION OF ASR
...
# isolate numeric
pcaDf_p_num<-pcaDf_p_Complete[,1:128]
...
test=merge(data,subjPvalues,by=c('subjectkey','eventname'))
```

```{r}
### FINISHED HERE 
saveRDS(subjPvalues,'~/subjPvalues.rds')
