...
# residential deprivation index removes most participants. Current form relegates us to post-hoc analyses in sub-sample, unless new data release is better populated. Commented out.
...
### not functional with residential deprivation commented out.
...
#pleColDescrip=ple[1,]
#
#### apply similar framework to extract purely retrospective
#
## note yle's are labeled ple's in the colnames, but end with a _y extension (ples end with _p extension)
#
###### Extract purely retrospective, i.e., viable for inclusion in tp1-based predictions
...
#
## verify with descriptions
#pleColDescrip[grep('_past_yr',pleColnames)]
## remove "past year?" from column names to boil down to binary variables of interest
...
#
## extract just tp1 (really timepoint 1.5, 1-year in) for ple's
#ple1=subset(ple,eventname=='1_year_follow_up_y_arm_1')
#
#
## well, can't use this until these three lines make combined sense. Could be administered to multiple parents, but odd that the vast majority of parents would have time to both fill #this measure but not others?
#length(unique(ple1$subjectkey))
#length(ple1$subjectkey)
#table(ple1$eventname)
