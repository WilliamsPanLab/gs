...
for (b in 1:1000){
	...
	gpAge_full<-gam(g~s(cbcl_scr_syn_totprob_r)+s(interview_age)+cbcl_q61_p,data=bootSamp)
	devExplained_full[b]<-summary(gpAge_full)$dev.expl
	# fit version with cbcl 61
	gpAge_nocbcl61<-gam(g~s(cbcl_scr_syn_totprob_r)+s(interview_age),data=bootSamp)
	devExplained_NoCbcl61[b]<-summary(gpAge_nocbcl61)$dev.expl
	# fit version with just cbcl61
	gpAge_noTotProbs<-gam(g~cbcl_q61_p+s(interview_age),data=bootSamp)
