...
for (b in 1:10000){
	...
	#### full model
	gpAge_full<-bam(g~s(cbcl_scr_syn_external_r)+s(interview_age)+Grades,data=bootSamp)
	...
	pgAge_full<-bam(cbcl_scr_syn_external_r~s(g)+s(interview_age)+Grades,data=bootSamp,family=nb())
	...
	# fit version with just Grades
	pgAge_noG<-bam(cbcl_scr_syn_external_r~Grades+s(interview_age),data=bootSamp,family=nb())
