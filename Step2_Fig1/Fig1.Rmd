...
Fits1=readRDS('~/Downloads/gp_CvSC_bootBetas_1k (1).rds')
...
Diffs1=readRDS('~/Downloads/gp_CvSC_diffs_1k (1).rds')
...
ggplot(FitsP_long, aes(x = Labels, y = Value, fill = Median)) +
  ...
  labs(title = expression(paste("\u03B2 for Total Problems"))) +
...
ggplot(FitsInt_long, aes(x = Labels, y = Value, fill = Median)) +
  ...
  labs(title = expression(paste("\u03B2 for Internalizing"))) +
  theme_minimal(base_size = 26)+theme(axis.title.y = element_blank())
...
ggplot(FitsExt_long, aes(x = Labels, y = Value, fill = Median)) +
  ...
  labs(title = expression(paste("\u03B2 for Externalizing"))) +
  theme_minimal(base_size = 26)+theme(axis.title.y = element_blank())
...
# load in data
Fits1=readRDS('~/Downloads/gpFitBoots_sRE_incEdu_1k2.rds')
...
# Prepare CIs for insertion
CIs <- data.frame(rep(seq(0, 161), 2), c(rep(1001, 162), rep(1002, 162)), percentiles_long$YValue, rep(1, (162*2)))
colnames(CIs) <- colnames(data_melt)
...
```{r}
# withdrawn depression for k - note WitBC and WitC derived chunks below
WFits=Fits1[,288:304]
MaxW=find_furthest_nonzero(WFits)
# melt data for plotting each line
data_melt <- melt(t(WFits))
data_melt$Var1 <- rep(seq(0, 16), nrow(WFits))
# Calculate percentiles
percentiles <- WFits %>%
summarise(across(everything(), quantile, probs = c(0.01, 0.99), na.rm = TRUE))
percentiles_long <- tidyr::pivot_longer(percentiles, cols = everything(), names_to = "Percentile", values_to = "YValue")
...
# Prepare CIs for insertion
CIs <- data.frame(rep(seq(0, 16), 2), c(rep(10001, 17), rep(10002,17)), percentiles_long$YValue, rep(1, (17*2)))
colnames(CIs) <- colnames(data_melt)
...
ggplot(data = data_melt2, aes(x = Var1, y = value, group = Var2)) +
  ...
  scale_alpha_manual(values = c(0.01, 1), guide = FALSE) + ylim(c(-1.5,1.5)) +
  theme_minimal(base_size=35) + 
  ylab(expression(italic(g)))+xlab('Withdrawn Depression')+
  geom_vline(xintercept = WitBc, linetype = "dashed")+
  geom_vline(xintercept = WitC, linetype = "dashed")+
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1)) +
  scale_x_continuous(limits = c(0,MaxW),expand = expansion(mult = c(0, 0)))
```
...
PFits=Fits[1:1000,1:162]
IFits=Fits[1:1000,163:214]
EFits=Fits[,181:228]
...
# actually plot em
plot_bootstraps(PFits,161,expression(italic(p)),MaxP,Pbc,Pc)
...
# find where each is 99% or greater
positive_countsSig=positive_counts>950
negative_countsSig=negative_counts>950
# make dataframe: 50th percentile of derivatives accompanied by posSig and NegSig vector
...
ggplot(data=dervPlotDf) + geom_raster(aes(x = seq, y = .5, fill = sig_deriv))+
      theme(panel.spacing = unit(-.01,"cm")) +
      scale_fill_gradientn(colors = my_palette(100),limits = c(min(-.1),max(0.1)))+theme_minimal(base_size = 35)+xlim(c(0,13))+xlab('Withdrawn Depression')+
      guides(fill=FALSE)+
      theme(axis.title.y = element_blank(),axis.text.y=element_blank())+theme(panel.border = element_rect(color = "black", fill = NA, size = 1))+
      scale_x_continuous(limits = c(0,13),expand = expansion(mult = c(0, 0)),breaks = seq(0, 13, by = 2))

```

```{r}
# New data for prediction
#new_data <- with(masterdf, expand.grid(
#  cbcl_scr_syn_internal_r = seq(min(cbcl_scr_syn_internal_r), max(cbcl_scr_syn_internal_r), length.out = 100),
#  income = unique(income),
#  interview_age = mean(interview_age), # Fixed average value for simplicity
#  site = 'site13', # Assuming a single level for simplicity
#  parental_education = 18 # Fixed average value
#))
#
## Predictions
#new_data$predicted <- predict(both_intgAge, newdata = new_data, type = "response")
#
## Plotting
#ggplot(new_data, aes(x = cbcl_scr_syn_internal_r, y = predicted, color = factor(income))) +
#  geom_line(size=5) +
#  labs(title = "Predicted values by cbcl_scr_syn_internal_r across income levels",
#       x = "CBCL Score",
#       y = "Predicted Value",
#       color = "Income Level") +
#  theme_minimal()
