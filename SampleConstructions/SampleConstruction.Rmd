...
# add sample split to included Subjects
includedSubjects$SampleSplit=0
includedSubjects[includedSubjects$subj %in% unique(masterdf$subjectkey[masterdf$matched_group==1]),]$SampleSplit=1
...
my_colors <- c("#F3756D", "#B59F31", "#28B34B", "#19BDC2", "#6F94CD", "#A5A5A5")

ggplot(test,aes(x=variable,stratum=RaceEthn,alluvium=subj))+geom_stratum(aes(fill=RaceEthn))+geom_flow(aes(fill=RaceEthn))+theme_bw(base_size=40)+theme(axis.text.x = element_text(angle=90))+scale_fill_manual(values=my_colors)
...
saveRDS(OutDFTmpPrec_nao[OutDFTmpPrec_nao$matched_group.x==1,],'~/OutDFTmpPrec_FullRetro.rds')
saveRDS(OutDFTmpPrec_nao[OutDFTmpPrec_nao$matched_group.x!=1,],'~/OutDFTmpPrec_FullRetro_heldout.rds')
```
...
# saveout
saveRDS(OutDFTmpPrec[OutDFTmpPrec$matched_group.x==1,],'~/OutDFTmpPrec_IVEandRetro.rds')
saveRDS(OutDFTmpPrec[OutDFTmpPrec$matched_group.x!=1,],'~/OutDFTmpPrec_IVEandRetro_heldout.rds')
```
...
# t1 include: thank you Safia, Keiko, and Josh!
SKJInclBV=read.csv('~/Downloads/Package_1211968/results/baseline_qc_mqcdef.csv')
# isolate timepoint1
...
# now include SKJ qc
colnames(SKJInclBV)[1]<-'subjectkey'
tempPrecSKJ=merge(tempPrecNDA,SKJInclBV,by='subjectkey')
# only want 1s and 2s
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=0,]
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=3,]
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=4,]
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=5,]
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=10,]
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=11,]
tempPrecSKJ=tempPrecSKJ[tempPrecSKJ$MQC_BL!=12,]
# add to alluvial df for plotting
includedSubjects$SKJ_QC=0
includedSubjects[includedSubjects$subj %in% unique(tempPrecSKJ$subjectkey),]$SKJ_QC=1
...
DL_list=gsub('R_I','RI',includedSubjects$subj[includedSubjects$SKJ_QC==1])
