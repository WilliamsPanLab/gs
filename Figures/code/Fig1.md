...
``` r
# for each bootstrap, recover median slope in bottom 50th percentile
df <- data.frame(
  p = apply(PFits[, 1:Pbc], 1, median),
  Internal = apply(IFits[, 1:Ibc], 1, median),
  External = apply(EFits[, 1:Ebc], 1, median),
  Somatic = apply(SomFits[, 1:SomBc], 1, median),
  AnxDepr = apply(AnxFits[, 1:AnxBc], 1, median),
  Thought = apply(ThoFits[, 1:ThoBc], 1, median),
  WithDepr = apply(WitFits[, 1:WitBc], 1, median),
  Social = apply(SocFits[, 1:SocBc], 1, median),
  Attn = apply(AttFits[, 1:AttBc], 1, median),
  Rules = apply(RulFits[, 1:RulBc], 1, median),
  Aggr = apply(AggFits[, 1:AggBc], 1, median)
)
...
ggplot(df_tidy, aes(x = Subscale, y = MedianValue,fill=MedianIteration)) +
  geom_boxplot() +
  labs(title = "Median Association with Cognitive Score: Subclinical Range",
       x = "Subscale",
       y = "Median Slope") +
...
df <- data.frame(
  ...
  Social = apply(SocFits[, -(1:SocBc)], 1, median),
  Attn = apply(AttFits[, -(1:AttBc)], 1, median),
  Rules = apply(RulFits[, -(1:AttBc)], 1, median),
  Aggr = apply(AggFits[, -(1:AggBc)], 1, median)
)
...
ggplot(df_tidy, aes(x = Subscale, y = MedianValue,fill=MedianIteration)) +
  geom_boxplot() +
  labs(title = "Median Association with Cognitive Score: Clinical Range",
       x = "Subscale",
       y = "Median Slope") +
  theme_minimal(base_size=20)+scale_fill_gradientn(
    colors = my_palette(100),
