...
ggplot(df_tidy_merged, aes(x = Subscale, y = MedianValue,fill=MedianIteration,outlier.shape=third)) +
    ...
    labs(title = "Cognitive association in low (left) and high (right) ranges",
         x = "Subscale",
         y = "Median Slope") +
    theme_minimal(base_size=30)+scale_fill_gradientn(
        colors = my_palette(100),
        limits = c(-.1,.1))+guides(fill=F)+theme(axis.text.x = element_text(angle = 35, hjust = 1),plot.title = element_text(size = 26),panel.border = element_rect(color = "black", fill = NA, size = 1))+ylim(c(-.225,.18))
```
