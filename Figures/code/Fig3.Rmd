...
F_PFits=Fits[,1:128]
F_IFits=Fits[,129:180]
F_EFits=Fits[,181:228]
...
```{r}
# isolate male fits
M_PFits=Fits[,229:356]
M_IFits=Fits[,357:408]
M_EFits=Fits[,409:456]
...
ggplot(data, aes(x = x, y = y_girls)) +
  geom_line(color = "#992965", size = 1.5) +
  geom_line(aes(y = y_boys), color = "#fbad24", size = 1.5) +
...
P_PFits=Fits[,457:584]

# calculate background info for plots
...
DevExpl=readRDS('~/F3-5DevExpl.rds')
DevExpl_L=readRDS('~/F3-5DevExpl_longit.rds')
...
```{r}
# poverty analysis
povBootsdf=readRDS('~/F3_gpPovNonPov.rds')
...
povBoots_subset <- povBoots[, 1:79]
nonpovBoots_subset <- nonpovBoots[, 1:79]
FullnonpovBoots_subset <- FullnonpovBoots[, 1:79]
