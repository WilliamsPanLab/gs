...
plot_bootstraps <- function(data,maxval,Name,maxValuePlot,BorderlineClinical,Clinical) {
  ...
  CIs <- data.frame(rep(seq(0, maxval), 2), c(rep(1001, maxval+1), rep(1002, maxval+1)), percentiles_long$YValue, rep(1, ((maxval+1)*2)))
...
# load in data
Fits=readRDS('~/Downloads/gpFitBoots_poly.rds')
# extract p factor
PFits=Fits[,1:128]
...
PFits=Fits[1:1000,1:128]
IFits=Fits[1:1000,129:180]
EFits=Fits[1:1000,181:228]
SomFits=Fits[1:1000,229:242]
AnxFits=Fits[1:1000,243:268]
ThoFits=Fits[1:1000,269:287]
WitFits=Fits[1:1000,288:304]
SocFits=Fits[1:1000,305:322]
AttFits=Fits[1:1000,323:342]
RulFits=Fits[1:1000,343:361]
AggFits=Fits[1:1000,362:394]
...
```{r}
# load in data
Fits=readRDS('~/Downloads/gpDerivBoots_poly.rds')
# find mean shape and plot it: p
PFits=Fits[1:1000,1:126]
IFits=Fits[1:1000,128:178]
EFits=Fits[1:1000,179:225]
SomFits=Fits[1:1000,229:242]
AnxFits=Fits[1:1000,243:268]
ThoFits=Fits[1:1000,269:287]
WitFits=Fits[1:1000,288:304]
...
ggplot(df_tidy_merged, aes(x= Subscale, y = MedianValue,fill=MedianIteration,outlier.shape=third)) +
    ...
    labs(title = "Cognitive association in subclinical (left) and clinical (right) ranges",
         x = "Subscale",
         y = "Median Slope") +
    theme_minimal(base_size=30)+scale_fill_gradientn(
        colors = my_palette(100),
        limits = c(-.1,.1))+guides(fill=F)+theme(axis.text.x = element_text(angle = 35, hjust = 1),plot.title = element_text(size = 27),panel.border = element_rect(color = "black", fill = NA, size = 1))+ylim(c(-.225,.18))
```
